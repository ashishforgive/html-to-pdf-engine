...
var RendererV5 = class {
  ...
  async renderToCanvas() {
    ...
    if (debug) {
      console.log("\u{1F4D0} RenderV5.1 size:", {
        width,
        height,
        marginPx,
        scale,
  ...
  // Render a node
  // -------------------------------------------------------------
  async renderNode(ctx, node, offsetX, offsetY, rootRect) {
  ...
  // -------------------------------------------------------------
  drawText(ctx, text, style, x, y, maxWidth, maxHeight) {
    if (!text.trim()) return;
    ...
    const lineHeightPx = style.lineHeight && style.lineHeight !== "normal" ? parseFloat(style.lineHeight) : parseFloat(fontSize) * 1.4;
    const words = text.trim().split(/\s+/);
    let line = "";
    let cursorY = y + lineHeightPx;
  ...
  drawBorders(ctx, style, x, y, w, h) {
    ...
    ctx.strokeStyle = style.borderColor || "#e5e7eb";
